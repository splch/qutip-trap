"""The explain drawer's content: the provenance index carries every section's own Markdown (Part II and the rest of the plan
the screens cite), every level, page, concept, catalogue quantity and preset resolves to a section whose text (or whose
subsections) the drawer can show, and a chip knows which section to open."""
...
def test_every_governing_section_resolves_to_text() -> None:
    idx = ProvenanceIndex.load()
    numbers = set(LEVEL_SECTIONS.values()) | {p.section for p in DEVICE_PAGES.values()}
