"""The device knobs of Level 4 and how a change propagates downward (PLAN.md Section 14.4; milestone M11.3).

Section 14.4: "The device model at Level 4 is the single source of truth. A change there, a trap voltage, a beam power, a
detection window, re-derives the analytic layer immediately ... and invalidates the calibration table". A knob is one named
device parameter with its plain label, physics term, unit, the Part II section and the ledger record that governs it, and a
range the page offers; an **override** is ``{knob id: value}``. :func:`apply_overrides` rebuilds a public preset with the
overrides applied in a fixed order, re-solving the crystal when the trap changed (``Device`` does not re-derive on
``replace``) and re-deriving the preparation recipe when the trap, the field or the beams changed (the recipe's Doppler
detuning is optimized over the mode structure, Section 4.2.1). The overrides travel in the job's ``DeviceRef`` so that a
record made on an edited device rebuilds it, hash-checked (Section 7.5: a table is invalidated, never silently reused).

Two conventions this module fixes:

- On the ``explicit`` trap path (a trap declared by its secular frequencies, the public presets' case) the rf amplitude has
  no voltage to scale, so ``trap.rf_amplitude_scale`` scales the Mathieu q of both radial axes at fixed a (q is proportional
  to V_rf, a to the dc voltages, Section 4.1.1) and lets beta follow from the Mathieu equation by the core's monodromy, so
  the radial secular frequencies move as nu = beta Omega_rf/2 and not simply in proportion to V_rf: the axial dc confinement
  gives the radial axes a negative a, which is why the knob needs the rf drive frequency declared (``trap.rf_frequency_hz``)
  and refuses without it rather than assuming a = 0. On a geometry path it scales ``RfDrive.voltage_peak_v`` itself.
- The Mathieu view of the trap page (a, q, beta, C0) needs an rf frequency; ``trap.rf_frequency_hz`` declares the rf record
  on a trap that has none (``Trap.mathieu`` uses ``mathieu_from_secular`` there, Section 4.1.1), with a peak voltage the
  page can also set. Declaring the record changes the device hash, as any parameter does.
"""
...
_FIXED: tuple[Knob, ...] = (
    _k(
        "trap.rf_amplitude_scale",
        "trap",
        "Radio-frequency drive strength, relative to the preset",
        "V_rf/V_rf,0 (q proportional to V_rf at fixed a; beta by the Mathieu equation)",
        "",
        "conv.rf_amplitude_pseudopotential",
        "4.1.1",
        0.5,
        1.5,
        "scales q of both radial axes at fixed a; beta and the radial secular frequencies follow from the Mathieu equation, the radial modes move, eta follows 1/sqrt(omega) times the micromotion factor C0 of the Mathieu record, the MS pulse must be re-solved and the table goes stale; needs the rf drive frequency",
        relative=True,
        requires_rf=True,
    ),
    _k(
        ...
        "omega_z/2pi (the dc end-cap stiffness)",
        "Hz",
        "conv.rf_amplitude_pseudopotential",
        "4.1.2",
        2e5,
        3e6,
        "moves the ions closer or farther apart (spacing goes as omega_z^-2/3) and rescales every axial mode",
        log=True,
    ),
    _k(
        "trap.rf_frequency_hz",
        "trap",
        "Radio-frequency drive frequency",
        "Omega_rf/2pi",
        "Hz",
        "conv.mathieu_sign",
        "4.1.1",
        1e7,
        1e8,
        "declares the rf record: a, q, beta and the micromotion factor C0 follow from the secular frequencies at this drive frequency",
        log=True,
    ),
    _k(
        "trap.rf_voltage_peak_v",
        "trap",
        "Radio-frequency peak voltage",
    ...
    _k(
        "trap.stray_field_y_v_per_m",
        "trap",
        "Stray electric field along y",
        "E_dc,y",
        "V/m",
        "anchor.trap.berkeland_excess_micromotion",
        "4.1.1",
    ...
    _k(
        "trap.axis_angle_rad",
        "trap",
        "Rotation of the radial trap axes",
        "axis angle about z",
        "rad",
        "conv.mode_index",
        "4.1.3",
    ...
    _k(
        ...
        2e-6,
        2e-4,
        "more photons separate bright from dark, but the off-resonant pumping has longer to flip the ion: the error has an interior optimum",
        log=True,
    ),
    _k(
        "detector.efficiency",
        "readout",
        "Fraction of scattered photons the detector counts",
        "eps_sys",
        "",
        ...
        0.002,
        0.3,
        "scales the detected bright rate once (never the leakage rates), so the count histograms separate faster",
        log=True,
    ),
    _k(
    ...
    _k(
        "noise.s_e_white",
        "noise",
        "Electric-field noise density at the ions",
        "S_E (two-sided, white)",
        "(V/m)^2/(rad/s)",
        ...
        1e-16,
        1e-8,
        "heats every mode at n_dot = e^2 S_E/(4 m hbar omega): the heating collapse operators appear with these rates",
        log=True,
    ),
    _k(
        "noise.correlation_length_m",
        "noise",
        "Distance over which the field noise is the same at two ions",
    ...
    _k(
        ...
        1e-26,
        1e-16,
        "dephases each qubit at gamma_phi = 2 pi^2 (dnu/dB)^2 S_B: nothing for a clock qubit at low field, everything for a Zeeman qubit",
        log=True,
    ),
    _k(
        "noise.rabi_drift_rms",
        "noise",
        "Shot-to-shot spread of the laser power",
        "rms of dOmega/Omega (quasi-static)",
        "",
        "conv.noise_routing",
        "6.4",
        0.0,
    ...
    _k(
        "preparation.doppler_duration_s",
        "cooling",
        "How long Doppler cooling runs",
        "t_Doppler",
        "s",
        ...
        1e-4,
        1e-2,
        "the Doppler stage reaches its rate-equation steady state in a few 1/W_m; longer only costs time",
        log=True,
    ),
    _k(
        "preparation.pump_duration_s",
        "cooling",
        "How long the optical pump runs",
        "t_pump",
        "s",
        "conv.preparation_stage_order",
        "4.2.6",
        2e-6,
        2e-4,
        "too short leaves population outside |0>: the preparation error and the pumping recoil follow from the master equation",
        log=True,
    ),
)
...
_BEAM_TEMPLATES: tuple[tuple[str, str, str, str, str, str, float, float, bool, str], ...] = (
    ...
    (
        "waist_m",
        "Beam {k} focus size",
        "w_0 of beam {k} (1/e^2 intensity radius)",
        "m",
        "conv.rabi_from_intensity",
        "4.5.4",
        1e-6,
...
def knob(id: str, device: core.Device | None = None) -> Knob:
    for k in _FIXED:
        if k.id == id:
            return k
    m = _BEAM_ID.match(id)
...
def apply_overrides(
    ...
    trap = dev.trap
    trap_changed = False
    omega = None if trap.omega_hz is None else [float(x) for x in trap.omega_hz]
    rf = trap.rf
    stray = [float(x) for x in trap.stray_field_v_per_m]
    axis = float(trap.axis_angle_rad)
    if "trap.omega_z_hz" in ov:
        if omega is None:
            raise KnobError("trap.omega_z_hz applies to a trap declared by its secular frequencies")
        if ov["trap.omega_z_hz"] <= 0.0:
            raise KnobError("trap.omega_z_hz must be positive")
    ...
    if "trap.rf_frequency_hz" in ov or "trap.rf_voltage_peak_v" in ov:
        ...
        if freq <= 0.0 or volt <= 0.0:
            raise KnobError("the rf frequency and peak voltage must be positive")
    ...
    if "trap.rf_amplitude_scale" in ov:
        scale = ov["trap.rf_amplitude_scale"]
        if scale <= 0.0:
            raise KnobError("trap.rf_amplitude_scale must be positive")
        if rf is None:
            raise KnobError(
        ...
        if trap.path == "explicit":
            ...
            omega = list(_scaled_radial_frequencies(probe, dev.crystal.species[0], scale))
        else:
            rf = dataclasses.replace(rf, voltage_peak_v=rf.voltage_peak_v * scale)
        trap_changed = True
    if trap_changed:
        trap = dataclasses.replace(
            trap,
            omega_hz=None if omega is None else (omega[0], omega[1], omega[2]),
            rf=rf,
            stray_field_v_per_m=(stray[0], stray[1], stray[2]),
            axis_angle_rad=axis,
        )
    ...
    if "field.b_gauss" in ov:
        if ov["field.b_gauss"] < 0.0:
            raise KnobError("field.b_gauss is a magnitude")
        field = dataclasses.replace(field, B_gauss=ov["field.b_gauss"])
        field_changed = True
    beams = list(dev.beams)
    beams_changed = False
    for key, value in ov.items():
        m = _BEAM_ID.match(key)
        if not m:
            continue
        k, name = int(m.group(1)), m.group(2)
        if value <= 0.0:
            raise KnobError(f"{key} must be positive")
        if name == "power_w":
    ...
    if "detector.window_s" in ov:
        if ov["detector.window_s"] <= 0.0:
            raise KnobError("detector.window_s must be positive")
        detector = dataclasses.replace(detector, window_s=ov["detector.window_s"])
    if "detector.efficiency" in ov:
        if not 0.0 < ov["detector.efficiency"] <= 1.0:
            raise KnobError("detector.efficiency lies in (0, 1]")
        detector = dataclasses.replace(detector, efficiency=ov["detector.efficiency"])
    if "detector.background_cps" in ov:
        if ov["detector.background_cps"] < 0.0:
            raise KnobError("detector.background_cps is non-negative")
    ...
    if "noise.s_e_white" in ov:
        level = ov["noise.s_e_white"]
        if level < 0.0:
            raise KnobError("noise.s_e_white is non-negative")
        noise_changes["S_E"] = core.white_spectrum(level, noise.S_E.unit)
    ...
    if "noise.correlation_length_m" in ov:
        if ov["noise.correlation_length_m"] < 0.0:
            raise KnobError("noise.correlation_length_m is non-negative")
        noise_changes["correlation_length_m"] = ov["noise.correlation_length_m"]
    if "noise.s_b_white" in ov:
        level = ov["noise.s_b_white"]
        if level < 0.0:
            raise KnobError("noise.s_b_white is non-negative")
        unit = noise.S_B.unit if noise.S_B is not None else "T^2/(rad/s)"
        noise_changes["S_B"] = core.white_spectrum(level, unit)
    if "noise.rabi_drift_rms" in ov:
        if ov["noise.rabi_drift_rms"] < 0.0:
            raise KnobError("noise.rabi_drift_rms is non-negative")
    ...
    crystal = dev.crystal
    if trap_changed:
        crystal = core.solve_crystal(trap, dev.crystal.species)
    ...
    if recipe is not None and rederive_recipe and (trap_changed or field_changed or beams_changed):
    ...
    if recipe is not None:
        if "preparation.doppler_duration_s" in ov:
            if ov["preparation.doppler_duration_s"] <= 0.0:
                raise KnobError("preparation durations must be positive")
        ...
        if "preparation.pump_duration_s" in ov:
            if ov["preparation.pump_duration_s"] <= 0.0:
                raise KnobError("preparation durations must be positive")
