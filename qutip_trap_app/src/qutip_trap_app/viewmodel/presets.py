"""The published-experiment presets of PLAN.md Section 14.5 ("Presets from the validation suite"; Section 9.11 row "Presets";
milestone M11.4), as pure data and pure functions: which Section 9 entries the app runs, the published numbers each places
beside the simulated ones with their provenance, and the comparison rule.

Every published value carries a ``published.*`` ledger record whose tag is the Section 9 row's tag for the SOURCE (what the
research runs verified about the paper's number), and every simulated value the ``anchor.*`` record of the check script that
recomputed it, so the two chips beside each other say different things and both are true. A preset also says whether the
simulated number is expected to agree: Section 9's rule is that consistency anchors report against the published uncertainty
or 20 percent, and the ledger's corrected forms name the cases (Kirchmair's fidelity, Myerson's optimum) where the
first-principles number is NOT the measured one and why; a learning tool that showed only agreements would teach the wrong
lesson about validation (DESIGN.md Section 10).

The computations themselves live in :mod:`qutip_trap_app.presets` (they import the core); this module imports neither the
core nor Flet, so the specs are testable as data.
"""
...
"""Section 9's default band for a consistency anchor: the published uncertainty (two sigma) or 20 percent, whichever is wider."""
...
class PublishedValue:
    ...
    """Catalogue id of the published number (its ledger record is a ``published.*`` one)."""
    quantity_simulated: str
    """Catalogue id of the simulated number (its ledger record is the ``anchor.*`` of the check that recomputed it)."""
    simulated_key: str
...
class PresetSpec:
    ...
    title: str
    question: str
    """What the learner will be able to answer after running it (the pedagogy rule: state the task, not the coverage)."""
    section: str
    row: str
    source: str
    ...
    concept_id: str
    """The learning-layer concept the explain drawer opens beside the result."""
    values: tuple[PublishedValue, ...]
    duration: str
    method: str
    """One sentence on what the simulator computes (shown in Details, never as body text)."""
    kind: PresetKind = "experiment"
    level: int = 4
    # circuit presets: what Level 0 loads and runs
    circuit_text: str = ""
    shots: int = 0
    n_ions: int = 0
...
PRESETS: dict[str, PresetSpec] = {
    ...
    for p in (
        PresetSpec(
            id="harty_2014",
            title="Harty 2014: one error in a million",
            question="why a microwave gate's error is set by its timing and detuning, not by the atom",
            section="9.2",
            row="Microwave RB",
            source="Harty et al. 2014",
            published_ledger_id="published.harty_2014_epg",
            simulated_ledger_id="anchor.m2.harty_randomized_benchmarking",
            concept_id="native_gate",
            values=(
                _v(
                    "epg_measured",
                    "error per gate, measured",
                    1.0e-6,
                    0.3e-6,
                    "",
                    "harty_epg",
                ...
                _v(
                    "epg_model",
                    "error per gate, the paper's own model",
                    0.81e-6,
                    0.14e-6,
                    "",
                    "harty_epg",
        ...
        PresetSpec(
            id="james_1998",
            title="James 1998: where the ions sit",
            question="how the equilibrium spacing and the axial mode frequencies follow from the Coulomb repulsion alone",
            section="9.1",
            row="Equilibrium positions; Axial modes",
            source="James 1998",
            published_ledger_id="published.james_1998_axial_modes",
            simulated_ledger_id="anchor.trap.james_spectrum",
            concept_id="mode",
            values=(
                _v(
                    "u_n2",
                    "outer ion position, two ions (units of l)",
                    (0.5) ** (2.0 / 3.0),
                    None,
                    "",
                    "james_u",
                ...
                _v(
                    "u_n3",
                    "outer ion position, three ions (units of l)",
                    (1.25) ** (1.0 / 3.0),
                    None,
                    "",
                    "james_u",
        ...
        PresetSpec(
            id="monroe_1995",
            title="Monroe 1995: how cold Doppler cooling gets",
            question="why Doppler cooling stops at a few quanta and what sets the number",
            section="9.3",
            row="Doppler anchors",
            source="Monroe et al. 1995",
            published_ledger_id="published.monroe_1995_doppler",
            simulated_ledger_id="anchor.m3.monroe_doppler_triple",
            concept_id="cooling_ladder",
            values=(
                _v(
                    "theory_112",
                    "the paper's theoretical occupation, 11.2 MHz mode",
                    0.484,
                    None,
                    "",
                    "monroe_nbar",
                ...
                _v(
                    "nbar_112",
                    "measured occupation, 11.2 MHz mode",
                    0.47,
                    0.05,
                    "",
                    "monroe_nbar",
                ...
                _v(
                    "nbar_112_rate",
                    "measured occupation against the rate framework, 11.2 MHz",
                    0.47,
                    0.05,
                    "",
                    "monroe_nbar",
                ...
                _v(
                    "nbar_182",
                    "measured occupation, 18.2 MHz mode",
                    0.30,
                    None,
                    "",
                    "monroe_nbar",
                ...
                _v(
                    "nbar_298",
                    "measured occupation, 29.8 MHz mode",
                    0.18,
                    None,
                    "",
                    "monroe_nbar",
                    "rate_298",
                    expect_agreement=False,
                    why_not="the triple is not reproduced by one beam (Section 9.3)",
                ),
            ),
            duration="under a second",
            method=(
        ...
        PresetSpec(
            id="roos_2000",
            title="Roos 2000: how hard a photon kicks a calcium ion",
            question="how the Lamb-Dicke parameter follows from the wavelength, the mass and the trap frequency",
            section="9.3",
            row="40Ca+ Lamb-Dicke recomputation",
            source="Roos 2000 (thesis)",
            published_ledger_id="published.roos_2000_lamb_dicke",
            simulated_ledger_id="anchor.ca40.lamb_dicke_729",
            concept_id="lamb_dicke",
            values=(
        ...
        PresetSpec(
            id="kirchmair_2009",
            title="Kirchmair 2009: an entangling gate on a hot ion",
            question="why the Molmer-Sorensen gate barely cares how hot the motion is, and what does limit it",
            section="9.4",
            row="Thermal populations",
            source="Kirchmair et al. 2009",
            published_ledger_id="published.kirchmair_2009_thermal_ms",
            simulated_ledger_id="anchor.m4.kirchmair_ca40",
            concept_id="spin_dependent_force",
            values=(
                _v(
                    "contrast",
                    "parity contrast at nbar = 20",
                    0.964,
                    None,
                    "",
                    "kirchmair_contrast",
                ...
                _v(
                    "fidelity_50",
                    "Bell-state fidelity at 50 us",
                    0.993,
                    0.001,
                    "",
                    "kirchmair_fidelity",
                    "fidelity_first_principles",
                    expect_agreement=False,
                    why_not=(
        ...
        PresetSpec(
            id="myerson_2008",
            title="Myerson 2008: reading a calcium qubit in 420 microseconds",
            question="why the readout error has a best window and a best threshold",
            section="9.5",
            row="40Ca+ optimum",
            source="Myerson et al. 2008",
            published_ledger_id="published.myerson_2008_readout",
            simulated_ledger_id="anchor.m5.myerson_optimum_and_recursion",
            concept_id="readout_rates",
            values=(
                _v(
                    "eps_at_point",
                    "error at n_c = 5.5, t_b = 420 us",
                    1.8e-4,
                    0.1e-4,
                    "",
                    "myerson_eps",
        ...
        PresetSpec(
            id="crain_2019",
            title="Crain 2019: reading ytterbium with a superconducting detector",
            question="how the readout error of this app's own detector compares with the apparatus it was modelled on",
            section="9.5",
            row="SNSPD operating point",
            source="Crain et al. 2019",
            ...
            values=(
                _v(
                    "infidelity",
                    "readout error, first-photon protocol",
                    6.9e-4,
                    0.6e-4,
                    "",
                    "crain_eps",
                ...
                _v(
                    "window",
                    "average detection time of the protocol",
                    11e-6,
                    None,
                    "s",
                    "crain_window",
        ...
        PresetSpec(
            id="bell_check",
            title="The Bell state, every piece of physics on",
            question="how far a two-ion Bell state lands from the ideal one on this machine, and why",
            section="9.6",
            row="Two-ion Bell state with all physics on",
            source="check_circuits.py (Appendix D)",
            published_ledger_id="anchor.m6.bell_state",
            simulated_ledger_id="anchor.m6.bell_state",
            concept_id="target_vs_simulated",
            values=(
                _v(
                    "p00",
                    "P(00), the check script's 4000 shots",
                    0.5015,
                    0.0079,
                    "",
                    "bell_check_probability",
                    "p00",
                ),
                _v(
                    "p11",
                    "P(11), the check script's 4000 shots",
                    0.4965,
                    0.0079,
                    "",
                    "bell_check_probability",
                ...
                _v(
                    "infidelity",
                    "register infidelity 1 - F",
                    2.336e-3,
                    None,
                    "",
                    "bell_check_infidelity",
                    "infidelity",
                    scale=1e3,
                ),
            ),
            duration="about 20 s (a full JOINT_EXACT run)",
            method="the two-ion Bell circuit through run() at the full engine, compared with the committed check script's numbers",
            kind="circuit",
            level=0,
            circuit_text=BELL_QASM_4000,
            shots=400,
            n_ions=2,
        ),
        PresetSpec(
            id="ghz_three",
            title="Three ions, one GHZ state",
            question="what changes when a third ion joins: more modes, a frozen zigzag, a smaller register fidelity",
            section="9.6",
            row="Three- and four-ion GHZ",
            source="check_benchmarks.py 5 (Appendix D)",
            published_ledger_id="anchor.m10.ghz_fidelity",
            simulated_ledger_id="anchor.m10.ghz_fidelity",
            concept_id="entanglement_by_ms",
            values=(
                _v(
                    "p000",
                    "P(000), the check script's 400 shots",
                    0.545,
                    0.025,
                    "",
                    "ghz_check_probability",
                    "p000",
                ),
                _v(
                    "p111",
                    "P(111), the check script's 400 shots",
                    0.4475,
                    0.025,
                    "",
                    "ghz_check_probability",
            ...
            method=(
                ...
                "engine, compared with the committed benchmark script's numbers"
            ),
            kind="circuit",
            level=0,
            circuit_text=GHZ3_QASM,
            shots=400,
            n_ions=3,
            preset_kwargs={"address_waist_m": 2.0e-6},
        ),
    )
}
...
class PresetResult:
    ...
    computation has one, the chart, and the parameters it used."""
...
class Comparison:
    ...
    difference: float
    """simulated - published, in the value's own unit."""
    sigmas: float | None
    """|difference| over the combined uncertainty, when either side has one."""
    within: bool
    """Inside the Section 9 band: ``SIGMAS`` combined sigmas or ``AGREEMENT_FRACTION`` of the published value, whichever is wider."""
    expect_agreement: bool
    verdict: str
    """One short line: "agrees to 2.9 %" or "differs by 0.4e-4 (4.3 sigma): why"."""
    why_not: str
    scale: float
    unit: str
...
def compare(spec: PresetSpec, result: PresetResult) -> tuple[Comparison, ...]:
    ...
    for v in spec.values:
        ...
        sigmas = abs(diff) / combined if combined > 0.0 else None
        band = max(SIGMAS * combined, AGREEMENT_FRACTION * abs(v.value))
        within = abs(diff) <= band
        rel = abs(diff) / abs(v.value) if v.value != 0.0 else float("inf")
        sig = "" if sigmas is None else f" ({sigmas:.1f} sigma)"
...
def circuit_comparisons(
    ...
    """A circuit preset's check-script numbers beside the run's own histogram and register fidelity (Level 0)."""
    simulated: dict[str, float] = {}
    unc: dict[str, float] = {}
    for key, p in probabilities.items():
        simulated[f"p{key}"] = float(p)
        unc[f"p{key}"] = float(error_bars.get(key, 0.0))
    if register_fidelity is not None:
        simulated["infidelity"] = 1.0 - float(register_fidelity)
    result = PresetResult(spec.id, simulated, unc, (), {}, (), 0.0)
    return compare(spec, result)
