"""Native gates, exactly (PLAN.md Section 7.6).

These matrices define what a native gate is supposed to do; the simulator never applies them to a state. Every function
takes RADIANS; IonQ's turns (1 turn = 2 pi) are converted with :func:`rad_from_turns` at the boundary.

- GPi(phi)  = [[0, e^{-i phi}], [e^{i phi}, 0]]
- GPi2(phi) = (1/sqrt 2)[[1, -i e^{-i phi}], [-i e^{i phi}, 1]] = exp[-i (pi/4)(cos phi X + sin phi Y)]
- MS(phi0, phi1, theta) = exp[-i (theta/2) GPi(phi0) (x) GPi(phi1)], fully entangling at theta = pi/2 (0.25 turns)
- ZZ(theta) = exp(-i (theta/2) Z (x) Z)
- RZ(theta) = diag(e^{-i theta/2}, e^{i theta/2}), virtual: phi -> phi - theta on every later gate phase, gates read in
  time order (IonQ's documentation states +theta in matrix order).
...
Tensor order (the one statement of it): in every two-qubit matrix here the gate's first qubit is the left Kronecker
factor, the most-significant index bit, so MS(phi0, phi1, theta) has GPi(phi0) on the left factor and the basis reads
|q_first q_second>. Qiskit's little-endian convention puts a gate's first qubit on the RIGHT factor, so the matrix of
``qiskit_ionq.MSGate(phi0, phi1, theta)`` (turns) is the SWAP conjugate of :func:`ms`, ms = SWAP @ M_qiskit @ SWAP.
"""
...
PAULI_X: Final[np.ndarray] = np.array([[0.0, 1.0], [1.0, 0.0]], dtype=complex)
PAULI_Y: Final[np.ndarray] = np.array([[0.0, -1.0j], [1.0j, 0.0]], dtype=complex)
PAULI_Z: Final[np.ndarray] = np.array([[1.0, 0.0], [0.0, -1.0]], dtype=complex)
IDENTITY_2: Final[np.ndarray] = np.eye(2, dtype=complex)
...
def gpi(phi_rad: float) -> np.ndarray:
    """GPi(phi): a pi rotation about the equatorial axis at azimuth phi."""
...
def xx(chi_rad: float) -> np.ndarray:
    """XX(chi) = exp(-i chi sigma_x (x) sigma_x); chi = pi/4 is maximally entangling."""
    c = math.cos(chi_rad)
    s = math.sin(chi_rad)
    return np.array(
...
def ms(phi0_rad: float, phi1_rad: float, theta_rad: float) -> np.ndarray:
    """MS(phi0, phi1, theta) = cos(theta/2) 1 - i sin(theta/2) GPi(phi0) (x) GPi(phi1) (the generator squares to 1)."""
...
def virtual_z_frame_shift(phi_rad: float, theta_rad: float) -> float:
    """The frame update of RZ(theta) on every later pulse phase, gates in time order: phi -> phi - theta."""
    return phi_rad - theta_rad


def global_phase(a: np.ndarray, b: np.ndarray, *, atol: float = 1e-9) -> float | None:
    """alpha with a = e^{i alpha} b to ``atol`` (the ratio read at b's largest element), or None."""
...
def equal_up_to_global_phase(a: np.ndarray, b: np.ndarray, *, atol: float = 1e-12) -> bool:
    """True when a = e^{i alpha} b for some alpha (the compiler's verification criterion)."""
    a = np.asarray(a, dtype=complex)
    b = np.asarray(b, dtype=complex)
    return a.shape == b.shape and global_phase(a, b, atol=atol) is not None
