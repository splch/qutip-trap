"""Harty et al. 2014's microwave randomized benchmarking as the paper simulated it (PRL 113, 220501, and supplement): from
|up>, each computational gate is a Pauli (two pi/2 pulses, a frame rotation or a delay) then a pi/2 Clifford pulse, and
the error per gate is the mean sequence error over the gate count (Fig. 7, at the defaults: 0.81e-6 mean, 0.14e-6 spread
over 500 sets of 32 sequences). In the frame of the drive, delta = omega_drive - omega_0, sigma_z = |up><up| - |down><down|
and a delay tau is exp[+i (delta tau/2) sigma_z]."""
...
class HartyParameters:
    """The operating point of Harty et al. 2014's single-qubit randomized benchmarking and the errors it is simulated at."""
    ...
    """The microwave detuning from the bare qubit transition."""
    ac_zeeman_hz: float = 0.0
    """Transition shift while the microwaves are on (Harty: -1.0 Hz; 0 for the Fig. 7 conditions); pulses see
    detuning_hz - ac_zeeman_hz."""
...
def rotation(theta: float, nx: float, ny: float, nz: float) -> np.ndarray:
    """exp[-i (theta/2) n . sigma]; n need not be normalized."""
...
def pulse_propagator(
    ...
    *,
    ...
    """An imperfect pi/2 pulse at azimuth phi: exp[-i (theta/2)(sigma_phi - ((delta - delta_ac)/Omega) sigma_z)] with
    theta = (pi/2)(1 + eps_a)."""
...
def simulate_sequences(
    ...
    *,
    ...
    # frame offsets are multiples of pi, so four azimuths cover every pulse; each actual pulse carries its dead time
    actual = {
    ...
    for g in range(n_gates):
        ...
        # +-z: idle delay, then the frame rotates by pi (the ideal z rotation)
        zmask = (p == 4) | (p == 5)
    ...
    # the ideal state up to the frame, a z rotation that does not affect a z-basis target
    up = np.array([1.0, 0.0], dtype=complex)
    psi_ideal = np.einsum("sij,j->si", V, up)
    # bring the ideal Bloch vector to the target pole with 0, 1 or 2 pi/2 pulses
    z = np.real(np.einsum("si,ij,sj->s", np.conj(psi_ideal), _SZ, psi_ideal))
    target_sign = np.where(targets == 0, 1.0, -1.0)
    errors = np.zeros(n_seq)
    for s in range(n_seq):
        ops: list[int] = []
        if abs(z[s]) > 0.5:  # at a pole
            if z[s] * target_sign[s] < 0:  # flip: two pi/2 pulses about +x
                ops = [0, 0]
        else:
            best = None
            for k in range(4):
        ...
        u = U[s]
        for k in ops:
            # ``best`` is a physical azimuth (V already carries the frame flips): no further flip
            u = actual[k] @ u
        final = u @ up
