"""Closed forms of the two-qubit gate literature (PLAN.md Section 4.4) that the gate tests and the published
models compare against.

Frequencies are angular (rad/s), the per-tone (hbar Omega/2) convention holds, and S_alpha = sum_i sigma_alpha^i: the
bichromatic force is -(hbar eta Omega/2) S_phi (a^dag e^{i eps t} + h.c.) with Section 13's sign (the builder's is the
opposite, from the i of i eta(a + a^dag); only |alpha| is used downstream), the loop closes at eps t = 2 pi K, the
two-body angle on sigma sigma is pi K (eta Omega/eps)^2, and eta Omega/eps = 1/(2 sqrt K) is maximally entangling.
"""
...
# ---- the Molmer-Sorensen propagator
...
# ---- beyond the leading order (Roos 2008)
...
# ---- microwave-gradient gates (Srinivas 2021)
...
def hughes_gate_angle_derived(
    ...
    pi/4 (S^2 has eigenvalues 0 and 4); the printed form is twice this for the displayed Hamiltonian."""
    val, _err = quad(lambda t: omega_g_rad_s(t) ** 2 / (2.0 * delta_rad_s(t)), 0.0, tau_s, limit=400)
    return float(val)


# ---- Bermudez 2017 budgets
...
# ---- segment and constraint counting
...
def fang_crosstalk_unitary(theta: float, theta_13: float, theta_23: float, phi_beam: float) -> np.ndarray:
    ...
    crosstalk model of Fang et al. 2022: XX(theta) = exp(-i theta X1 X2) with no 1/2 (Section 13)."""
...
def landsman_parallel_gate_bound(inter_pair_phases_rad: Sequence[float]) -> tuple[float, bool]:
    ...
    load-bearing); returns the bound on (1/2)||E||_diamond and whether it is vacuous (above 1, since
