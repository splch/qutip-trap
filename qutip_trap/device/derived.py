...
quantity the device cannot derive (a microwave drive's Rabi frequency, a detection rate without a detection beam) is absent
...
def derived_quantities(device: Device) -> DerivedQuantities:
    ...
    if device.trap.rf is not None:
        ...
        # ValueError is the layer's "no rf record / outside the stability region" signal (UnstableMathieuError subclasses
        # it); any other exception is a bug and surfaces as one
        except ValueError as exc:
