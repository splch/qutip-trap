"""Noise spectra and slow-parameter records.

Every ``NoiseSpectrum`` is TWO-sided in angular frequency with the e^{-i omega t} kernel: its variance is
(1/pi) int_0^inf S d omega, and a laboratory's single-sided per-hertz density is 2 S(omega = 2 pi f).
"""
...
class NoiseSpectrum:
    """A power spectral density: always two-sided in angular frequency with the exp(-i omega t) kernel."""
    ...
    """The flat two-sided white density (unit of S) that ``value`` adds everywhere; it becomes a Lindblad operator."""
    provenance: tuple[str, ...] = ()
    """One entry per apparatus the density came from ("author year, species, N ions"); empty = undeclared."""
    ...
    def variance(self) -> float:
        """(1/pi) int_0^inf S_tab d omega: the variance of the tabulated (sampled) part, without the white level."""
        w = np.asarray(self.omega_rad_s, dtype=float)
        s = np.asarray(self.S, dtype=float)
        if np.any(w < 0.0):
...
def white_spectrum(level: float, unit: str) -> NoiseSpectrum:
    """A purely white process: a two-point zero band with the density in ``white_level``."""
...
def ou_spectrum(
    ...
    """The Ornstein-Uhlenbeck (Lorentzian) density S(omega) = 2 sigma^2 tau_c/(1 + omega^2 tau_c^2), autocorrelation
...
def gaussian_spectrum(
    ...
    """S(omega) = sqrt(2 pi) (rms^2/sigma) exp[-omega^2/(2 sigma^2)], so C(0) = rms^2 (default band: 8 sigma)."""
...
def power_law_spectrum(
    ...
    *,
    ...
    """S(omega) = S_ref (omega/omega_ref)^-alpha, log-spaced on [omega_min, omega_max]; omega_min is the IR cutoff."""
...
def spectrum_from_single_sided_hz(
    ...
    """A single-sided per-hertz density as a NoiseSpectrum: S_two(omega = 2 pi f) = S_single(f)/2, no further 2 pi."""
...
@dataclass(frozen=True)
class Drift:
    """A slow parameter (``rms`` and ``rate_per_s`` in the unit of the NoiseModel field that carries it): samples at
    shot-clock times t and t' are correlated as exp(-|t - t'|/tau_s), plus the ramp rate_per_s x (t - t0); a servo of
    ``servo_bandwidth_hz`` leaves only its residual."""
    ...
    provenance: tuple[str, ...] = ()
    """One entry per apparatus, as on ``NoiseSpectrum``."""
...
@dataclass(frozen=True)
class Mains:
    """Mains pickup: a magnetic-field amplitude and phase per harmonic of ``line_hz``, and the line-trigger policy."""
...
@dataclass(frozen=True)
class Collisions:
    """Background-gas collisions: pressure, gas mixture, outcome distribution given a collision and the kick model."""
    ...
    """The kick energy distribution at mean ``mean_kick_energy_j``: exponential, or Maxwell (chi-squared, 3 dof)."""
    kick_scale_multiplier: float = 1.0
    """Multiplies the k_B T m_gas/m_ion kick energy scale."""
    reorder_permutations: tuple[tuple[int, ...], ...] = ()
    """Candidate ion orders a reorder draws uniformly; empty = the struck ion swaps with its neighbour."""
