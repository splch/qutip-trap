"""Summaries of a simulated channel for circuit-level clients (PLAN.md Section 6.8).

Phenomenological channels are never used inside the simulator; they are what it reports. From a trace-1 Choi state: the
entanglement infidelity 1 - F_e and the average gate infidelity d/(d + 1)(1 - F_e) (Nielsen 2002); the Pauli twirl, whose
p_XX for exp(-i alpha XX) is sin^2 alpha (Trout 2018); and the depolarizing rate eps of
Lambda_eps(rho) = (1 - eps) rho + eps/(4^n - 1) sum_{P != I} P rho P, whose entanglement infidelity is exactly eps (Chen et
al. 2023). Also the randomized-benchmarking conversions: r = (1 - p)(2^n - 1)/2^n per Clifford, and (4^n - 1)(1 - p)/4^n
the entanglement infidelity of the depolarizing channel with the same p.
"""
...
def average_gate_infidelity(eps_entanglement: float, d: int) -> float:
    """1 - F_avg = d/(d + 1) (1 - F_e) (Nielsen 2002); 4/5 of it for two qubits."""
...
def depolarizing_kraus(eps: float, n_qubits: int) -> list[np.ndarray]:
    """The Kraus operators of Lambda_eps: sqrt(1 - eps) 1 and sqrt(eps/(4^n - 1)) P over the non-identity Paulis (Trout
    2018's uniform eps/3 and eps/15 weights), trace-preserving for every eps in [0, 1]."""
    if not 0.0 <= eps <= 1.0:
...
def qiskit_depolarizing_lambda(eps: float, n_qubits: int) -> float:
    """lambda = eps 4^n/(4^n - 1), the weight of the maximally mixed state that Qiskit's ``depolarizing_error`` takes for the
    channel whose uniform Pauli weight is eps/(4^n - 1)."""
    return float(eps * 4**n_qubits / (4**n_qubits - 1))


def depolarizing_choi(eps: float, n_qubits: int) -> np.ndarray:
    """The Choi state of Lambda_eps."""
    return choi_from_kraus(depolarizing_kraus(eps, n_qubits))


def depolarizing_rate(choi: np.ndarray, choi_ideal: np.ndarray) -> float:
    """The depolarizing rate eps, which is the entanglement infidelity by Chen et al.'s normalization."""
