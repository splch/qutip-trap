"""State-based process tomography, the CP/TP projection and Kraus application: the GATE_LOCAL channel of a pulse group
(PLAN.md Section 5.4).

The channel of a step on the addressed ions' internal states is extracted from its exact evolution on its own joint space,
one motional branch of the tracked motional state at a time, by one of three routes (``TomographyRecord.route``):

- "states": each of the prod_i d_i^2 linearly independent pure inputs of ``input_states`` is propagated and the Choi matrix
  fit by least squares (``choi_least_squares``); every dissipative step takes this route, with n_traj = ceil(1/epsilon_map)
  trajectories per input on the trajectory path;
- "isometry": a unitary step's final state is linear in its initial ket, so the prod_i d_i internal basis kets are
  propagated per branch and the channel read off the Stinespring isometry they form (``choi_from_isometry``);
- "propagator": on an internal-state-only space the segment propagator of each branch is its Kraus operator.

The Choi matrix (trace-1 convention of ``noise/summary.py``) is projected onto CP and TP by Dykstra's alternating
projection, because a PSD projection alone leaves Tr_out C != 1/d and a map that changes the register's norm; the register
is then updated by the Kraus operators on its local factors (``apply_kraus_dm``) or by Kraus sampling on one member of a
pure-state ensemble (``apply_kraus_ket``), and the motional outputs, linear in the input, give the reduced motional state
the register leaves (``TomographyRecord.motional_for``).
"""
...
"""How a channel was extracted (the module docstring)."""

_DYKSTRA_TOL = 1e-13
"""Dykstra's projection stops when an iteration moves the Choi matrix by less than this (Frobenius norm)."""
_DYKSTRA_MAX_ITER = 20000
_FINGERPRINT_DIGITS = 9
"""Decimals the tracked reduced states are rounded to in ``fingerprint_model``."""
...
def internal_basis(ion_dims: Sequence[int]) -> list[np.ndarray]:
    """The prod_i d_i computational-basis kets of the internal space in the order of ``computational_labels``: the columns
    the isometry route propagates."""
...
def choi_from_isometry(v: np.ndarray, d_int: int) -> np.ndarray:
    ...
    The columns of V are the propagated kets U(T)(|j> (x) |m_b>) of one motional branch (ions first, so a column reshapes to
    (internal out, motional out)); its slices by motional output a are the Kraus operators K_a, and C = (1/d_int) sum_a
    |phi_a><phi_a| with phi_a = sum_i |i> (x) K_a|i> (the ``noise.summary.choi_from_kraus`` convention) is formed as one
    product. Completely positive by construction; trace preserving up to the norm the columns lost."""
...
def tp_residual(choi: np.ndarray) -> float:
    """||d Tr_out(C) - 1||_F (Tr_out of the trace-1 C is 1/d for a TP map)."""
...
def project_cptp(choi: np.ndarray) -> tuple[np.ndarray, float, float]:
    """Dykstra's alternating projection of ``choi`` onto CP (PSD) and TP (affine): (C, cp_residual, tp_residual) of the
    returned matrix, which ends on the TP set so that the register's normalization is exact."""
    x = np.asarray(choi, dtype=complex)
    p = np.zeros_like(x)
    q = np.zeros_like(x)
    for _ in range(_DYKSTRA_MAX_ITER):
        y = project_psd(x + p)
        p = x + p - y
        ...
        x = x_new
        if delta < _DYKSTRA_TOL:
            break
    x = project_tp(x)
    x = 0.5 * (x + x.conj().T)
    return x, cp_residual(x), tp_residual(x)
...
def apply_kraus_dm(
    ...
    register density matrix over ``dims``: one product of the d_loc^2 x d_loc^2 superoperator with the register reshaped to
    (local row, local column) x (rest row, rest column)."""
    n = len(dims)
...
def motional_branches(
    ...
    *,
    ...
    weight >= ``weight_min`` kept and renormalized, the dropped weight reported. With ``dropped_weight_max`` the lightest
    branches go too while the total dropped weight stays within it (the tail rule): the channel is a convex mixture over
    the branches, so dropping weight w changes it by at most 2w in diamond norm."""
    notes: list[str] = []
    resolved_options: list[tuple[int, list[tuple[float, qt.Qobj]]]] = []
    for tr in space.resolved:
        ...
        opts_m: list[tuple[float, qt.Qobj]] = []
        ...
        resolved_options.append((m, opts_m))
    frozen_options: list[tuple[int, list[tuple[float, int]]]] = []
    for m in sorted(set(int(x) for x in coupled_frozen)):
        ...
        if rho is not None:
            ...
            pops = thermal_populations(nb, _thermal_levels(nb)) if nb > 0.0 else np.array([1.0])
        opts_f = [(float(p), int(n)) for n, p in enumerate(pops) if p >= weight_min]
        if not opts_f:
            opts_f = [(1.0, int(np.argmax(pops)))]
        frozen_options.append((m, opts_f))
...
class TomographyRecord:
    """The channel of one pulse group and what GATE_LOCAL tracks of it (Section 5.4)."""
    ...
    choi_raw: np.ndarray
    """Before the projection: the least-squares fit ("states") or the isometries' Choi matrix (the other routes)."""
    cp_residual: float
    tp_residual: float
    n_traj: int
    method: str
    ...
    engine_runs: int
    """Engine calls: d_int^2 per branch on the "states" route, d_int per branch on "isometry", one per branch on
    "propagator"."""
    notes: tuple[str, ...]
    integrators: tuple[str, ...]
    reports: tuple[EngineReport, ...]
    workers: int = 1
    """Processes the runs used (1 = everything in-process)."""
    ...
    tighter, weighted by its share (a bound on the diamond-norm change); None otherwise."""
    ...
    def motional_for(self, rho_local: np.ndarray) -> tuple[dict[int, np.ndarray], dict[int, complex]]:
        ...
        c = expansion_coefficients(rho_local, self.inputs)
    ...
    def residual_displacement(self) -> dict[int, float]:
        """Per resolved mode, max over the computational-basis inputs of |<a_m>| after the pulse (Section 5.4)."""
    ...
    def summary(self, ideal: np.ndarray | None = None) -> ChannelSummary:
        """The Section 6.8 summary against ``ideal``: the average gate infidelity, the Pauli twirl of the error channel
        E o U^dag and the depolarizing rate (the entanglement infidelity); without an ideal the twirl is of the channel itself
        and the infidelities are NaN."""
        d = self.dimension
...
def _branch_sample(sample: NoiseSample, branch: MotionalBranch) -> NoiseSample:
    """The sample of one motional branch: the frozen coupled modes' Fock states and the branch weight (which scales the
    boundary threshold) on top of the run's values."""
...
def _run_tasks(
    ...
    if opts.map == "serial" or workers <= 1 or len(payloads) < 2 or not space.resolved:
        ...
        for task in payloads:
            traces = engine.run_pulses(device, sched, task.state, space, task.sample, seeds, opts)
            rep = engine.last_report
            assert rep is not None
...
class _Extraction:
    ...
    n_traj: int = 1
    method: str = "sesolve"
    choi_raw: np.ndarray | None = None
    """The Choi matrix the isometry routes form directly; None on the "states" route (fit by least squares)."""
    notes: tuple[str, ...] = ()


class _Columns(NamedTuple):
    """The isometry route's runs: one D x d_int isometry per branch, the reports, the boundary populations the engine's
    monitor saw on the basis kets, the processes used, and the space a run grew to (None when none grew)."""
...
def _margins(reports: Iterable[EngineReport]) -> dict[int, int]:
    """Per resolved mode, the smallest margin any run kept above the populated range."""
    margins: dict[int, int] = {}
    for rep in reports:
...
def _isometry_columns(
    ...
    grown = _grown_space(reports, current)
    if grown is not None:
        return _Columns([], reports, {}, map_workers, grown)
    ...
    for task, (traces, _rep) in zip(payloads, results):
        ...
        for m, v in traces.boundary_population.items():
            boundary[m] = max(boundary.get(m, 0.0), float(v))
    return _Columns(columns, reports, boundary, map_workers, None)
...
def _propagator_columns(
    ...
    one engine call per branch and no state propagated."""
    columns: list[np.ndarray] = []
    reports: list[EngineReport] = []
    for br in branches:
        ...
        columns.append(u)
        reports.append(rep)
    return _Columns(columns, reports, {}, 1, None)
...
def _extract_from_columns(
    ...
    """The channel and the per-input quantities from one isometry V_b per branch, by linearity.

    The Choi matrix is sum_b w_b C(V_b); the output joint ket of input psi_k on branch b is V_b psi_k, reduced as the engine
    reduces its final states, so the outputs keep the meaning of the "states" route. The boundary populations the engine
    saw on the basis kets are folded with the inputs' final ones; a superposition's is at most d_int times the basis kets'
    (Cauchy-Schwarz), which the record notes.
    """
    d_int = int(np.prod(current.ion_dims))
    ...
    boundary = dict(runs.boundary)
    dims_ket = [list(current.dims), [1] * len(current.dims)]
    for br, v in zip(branches, runs.columns):
        w = br.weight
    ...
    if route == "propagator":
        note = (
            ...
            f"({len(branches)}); no state propagated"
        )
    else:
        note = (
    ...
    return _Extraction(
        route=route,
        outputs=[out_acc[k] for k in range(n_in)],
        mot_out={m: list(mats) for m, mats in mot_acc.items()},
        alpha_out={m: list(vals) for m, vals in alpha_acc.items()},
        boundary=boundary,
        margins=_margins(runs.reports),
        reports=runs.reports,
        runs=len(runs.reports),
        workers=runs.workers,
        choi_raw=choi_raw,
        notes=(note,),
    )
...
def tomography(
    ...
    # the tail rule's budget: the bound 2w stays inside half the map accuracy unless the caller set it
    tail_budget = (
    ...
    current = space
    for _attempt in range(_MAX_GROWTH_RETRIES + 2):
        ...
        if route == "states":
            ext_or_grown = _extract_from_states(
                engine, device, sched, current, branches, thermal_frozen, labels_kets, sample, seeds, opts
            )
            if isinstance(ext_or_grown, HilbertSpace):
                current = ext_or_grown
                continue
            ext = ext_or_grown
        else:
            if route == "propagator":
                columns = _propagator_columns(
                    engine, device, sched, current, branches, sample, seeds, opts, motional_model
                )
            else:
                columns = _isometry_columns(
                    engine, device, sched, current, branches, thermal_frozen, sample, seeds, run_opts
                )
            if columns.grown is not None:
                current = columns.grown
                continue
            ext = _extract_from_columns(current, branches, columns, labels_kets, route)
        tolerance_change: float | None = None
        probe_notes: list[str] = []
        if keyed is not None and columns is not None:
            # the dominant branch ten times tighter: d times the trace norm of its Choi change bounds the diamond norm
            probe = _isometry_columns(
                engine,
                device,
                sched,
                current,
            ...
            if probe.grown is None and probe.columns:
                d_int = int(np.prod(current.ion_dims))
                c_probe = choi_from_isometry(probe.columns[0], d_int)
                c_run = choi_from_isometry(columns.columns[0], d_int)
                tolerance_change = float(branches[0].weight) * d_int * _trace_norm(c_probe - c_run)
                ext = replace(ext, reports=ext.reports + probe.reports, runs=ext.runs + len(probe.reports))
                probe_notes.append(
        ...
        choi, cp_res, tp_res = project_cptp(choi_raw)
        ...
        if dropped > 0.0:
            notes.append(
                ...
                f"{dropped:.3e} (renormalized); channel error bound 2w = {2.0 * dropped:.3e}"
            )
        if current != space:
            notes.append(
                f"the truncation monitor grew the gate-local space from dims {space.dims} to {current.dims}"
            )
...
def fingerprint_model(model: MotionalModel, modes: Iterable[int]) -> tuple[object, ...]:
    """The motional model restricted to ``modes``: the tracked reduced states or the occupations, rounded."""
    out: list[object] = []
    for m in sorted(set(int(x) for x in modes)):
        rho = model.reduced.get(m)
        if rho is not None:
            out.append((m, np.round(np.asarray(rho.full()), _FINGERPRINT_DIGITS)))
        else:
            out.append((m, round(float(model.nbar.get(m, 0.0)), _FINGERPRINT_DIGITS)))
    return tuple(out)
