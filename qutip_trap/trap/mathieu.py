"""Mathieu equation, characteristic exponent, Floquet function and the micromotion factor C0.

Convention: d^2x/dxi^2 + [a - 2q cos 2xi] x = 0 with xi = omega_rf t/2, a = 4 Z|e| U alpha/(m omega_rf^2) and
q = 2 Z|e| U~ alpha'/(m omega_rf^2) (Leibfried et al. 2003); Wineland 1998 and Berkeland 1998 use +2q cos, a half-period
shift of the rf phase origin. beta comes from the monodromy matrix M over one period (cos(pi beta) = tr M/2, stable iff
|tr M| < 2); C0 = c_0 is the e^{i nu t} Fourier coefficient of the Floquet function under the Wronskian normalization
sum_n c_n^2 (1 + n omega_rf/nu) = 1, applied only in ``Crystal.lamb_dicke``.
"""
...
class UnstableMathieuError(ValueError):
    """The (a, q) point lies outside the stability region."""


# ---- closed-form checks on the monodromy result -------------------------------------------------------


def beta_lowest_order(a: float, q: float) -> float:
    """The lowest-order characteristic exponent beta ~ sqrt(a + q^2/2), a check on the monodromy result."""
    radicand = a + q * q / 2.0
    if radicand <= 0.0:
...
def c0_series(q: float) -> float:
    """C0 = 1 + 3 q^2/16, the O(q^2) micromotion factor on eta (even in q, no first-order term)."""
    return 1.0 + 3.0 * q * q / 16.0


def secular_rad_s(beta: float, omega_rf_rad_s: float) -> float:
    """nu = beta omega_rf/2, angular."""
    return beta * omega_rf_rad_s / 2.0


def pseudopotential_radial_rad_s(q: float, omega_rf_rad_s: float) -> float:
    """omega_r = q Omega/(2 sqrt 2), the a = 0 pseudopotential frequency with q the Mathieu parameter (Wineland 1998
    Eq. 6 writes q for the charge)."""
...
class Monodromy:
    ...
    @property
    def beta(self) -> float:
        """cos(pi beta) = tr M/2 on the principal branch beta in [0, 1]; raises outside the stability region. A marginal
        axis (|tr M| = 2 to round-off, e.g. unconfined a = q = 0) returns 0 or 1 for the crystal builder to refuse."""
...
def is_stable(a: float, q: float) -> bool:
    """Whether x'' + (a - 2q cos 2xi) x = 0 is stable at (a, q): |tr M| < 2 over one period."""
    return monodromy(a, q).stable


def stability_edge_q(a: float = 0.0, *, q_lo: float = 0.5, q_hi: float = 1.5) -> float:
    """The q at which beta reaches 1 for the given a (tr M = -2): 0.908 at a = 0."""
...
class FloquetCoefficients:
    ...
    i.e. Im(u* du/dxi) = beta in xi = omega_rf t/2; ``c`` are the real coefficients of the harmonics ``n``, c_0 > 0."""
    ...
    def sideband_weights(self) -> dict[int, float]:
        """|c_n/c_0|^2 per harmonic: the Floquet weights of the micromotion-sideband heating sum. At lowest order
        |c_{+-1}/c_0|^2 = q^2/16 is Turchette's omega_m^2/(2 Omega_T^2), never to be added to it; Brownnutt's index 2j is
        this n = j."""
    ...
    def comb_phase_per_rf_period_rad(self) -> float:
        """nu T_rf = beta pi: the secular phase u accumulates over one rf period, the phase step between successive teeth
        of the micromotion comb."""
        return float(self.beta * math.pi)

    def lowest_order_ratios(self) -> tuple[float, float]:
        """The lowest-order closed forms c_{+1}/c_0 = -q/((2 + beta)^2 - a) and c_{-1}/c_0 = -q/((2 - beta)^2 - a)."""
        return (
...
def floquet_coefficients(a: float, q: float, *, n_max: int = 24) -> FloquetCoefficients:
    """Fourier coefficients of the Floquet function at the monodromy beta: the null vector of the recursion
    [a - (beta + 2n)^2] c_n - q (c_{n-1} + c_{n+1}) = 0 truncated to |n| <= n_max, Wronskian-normalized."""
    beta = beta_exact(a, q)
...
def floquet_function_by_integration(
    ...
    """The Floquet eigen-solution u(xi) over one period by direct integration, Wronskian-normalized: (xi, u, beta). An
    independent construction of what ``floquet_coefficients`` builds from the recursion."""
    mono = monodromy(a, q)
    beta = mono.beta
...
def companion_matrix(
    ...
    """F(tau) = [[0, 1], [-(A - 2Q cos 2tau), 0]] for u'' + [A - 2Q cos 2tau] u = 0; ``lower_left_sign`` = +1 reproduces
    House 2008's printed Eq. 7, whose sign is wrong."""
    n = A.shape[0]
    f = np.zeros((2 * n, 2 * n))
    f[:n, n:] = np.eye(n)
...
class MathieuParameters:
    """a, q matrices, exponents, secular frequencies and C0 per principal axis. ``a`` and ``q`` are in the pseudopotential's
    principal-axis frame, whose directions ``axes`` holds as laboratory-frame columns; the per-axis tuples follow that
    (x', y', z) order."""
    ...
    """Wronskian-normalized Floquet coefficient c_0 per axis."""
    sign_convention: Literal["a - 2q cos 2xi"] = MATHIEU_SIGN_CONVENTION
    axes: np.ndarray | None = None
    """None means the laboratory axes."""
    omega_rf_hz: float | None = None
    mass_kg: float | None = None
    """The ion mass the parameters were computed for; a and q scale as 1/m at fixed voltages, which is how
    ``Crystal.c0_for`` derives another species' C0."""
...
def _principal_frame(A: np.ndarray, Q: np.ndarray) -> np.ndarray:
    ...
    if commute:
        # commuting A and Q share eigenvectors, so a generic c Q breaks degeneracies of A + Q^2/2 (a_x = a_y with
        # q_y = -q_x) without moving them; only the eigenvectors are read here
...
def mathieu_parameters(
    ...
    *,
    ...
    """Exponents, secular frequencies and C0 from per-axis (a, q) triples or 3 x 3 laboratory-frame matrices; non-diagonal
    matrices are rotated into the principal frame of A + Q^2/2 and use the vector monodromy, and C0 from
    (a_ii, sqrt((Q^2)_ii)) is then a pseudopotential-frame approximation unless A and Q commute."""
    if omega_rf_hz <= 0.0:
    ...
    # the rf strength along a principal axis is sqrt((Q^2)_ii), |q_ii| when Q is diagonal in this frame
    q2 = q_p @ q_p
...
def mathieu_from_secular(
    ...
    """Invert the exact exponents for (a_x, a_y, a_z) and (q_x, -q_x, 0) from measured secular frequencies (x, y, z),
    imposing the linear-trap structure q_z = 0, q_y = -q_x with q_x >= 0, and a_x + a_y + a_z = 0."""
    wx, wy, wz = omega_hz
    if min(omega_hz) <= 0.0:
