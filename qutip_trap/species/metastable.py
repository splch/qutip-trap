"""Metastable-level channels that default off (PLAN.md Section 4.5.7): physical inputs, never fudge factors.

- Blackbody M1 mixing between the fine-structure D levels, W_12 = A_12 n_bar(nu, T) with the Bose occupation
  n_bar = 1/(exp(h nu/k_B T) - 1) MULTIPLIED (dividing by it is low by n_bar^2), the upward rate carrying g_u/g_l.
  For 40Ca+ (A_12 = 2.45e-6 s^-1, Ali and Kim 1988) it is 7.249e-6 s^-1 at 300 K, 8.5e-6 of the natural decay rate.
- Collisional quenching and j-mixing by the background gas, R = sum_s Gamma_s p_s/(k_B T) with Gamma_s a cited
  coefficient in cm^3 s^-1 (Knoop, Vedel and Vedel 1995 via Kreuter et al. 2005); an uncited partner raises.
- Reshelving, p_D' = -Gamma p_D + R(1 - p_D), whose long-delay offset R/(Gamma + R) is its experimental signature.

The rates enter a readout's shelf model as extra loss channels (``effective_shelf_lifetime_s``) and any D-manifold master
equation as sqrt(R) collapse-operator rates.
"""
...
"""The two Knoop et al. 1995 processes; a table cites them as ``D.collision_<process>_<gas>_cm3_s``."""
...
def bbr_mixing_rates_hz(
    ...
    """(downward A n_bar, upward (g_u/g_l) A n_bar) in s^-1 for a line of Einstein coefficient ``a_s`` at ``nu_hz``.

    Kreuter et al. 2005 Eq. 1: W_12 = A_12/(e^{h nu/kT} - 1); the upward rate follows from detailed balance of the two
    stimulated rates, B_lu rho = (g_u/g_l) B_ul rho with B_ul rho = A n_bar.
    """
    if a_s < 0.0:
...
class MetastableChannels:
    ...
    """None disables blackbody D-D mixing."""
    ...
    """{"H2": 0.5, "N2": 0.5}, summing to 1 when collisions are enabled."""
    reshelving_rate_hz: float = 0.0
    """R of p_D' = -Gamma p_D + R(1 - p_D) in s^-1; exposed, never fitted away."""
    gas_temperature_k: float = 300.0
    """The background-gas temperature in n_s = p_s/(k_B T): the rates are density driven, not pressure driven."""
    ...
    def bbr_rate_hz(self, transition: str, species: Species) -> tuple[float, float]:
        ...
        ``species``: its Einstein coefficient is the transition's partial rate, its frequency the level spacing;
        (0, 0) when the channel is disabled."""
        if self.bbr_temperature_k is None:
            return 0.0, 0.0
        tr = species.transition(transition)
        return bbr_mixing_rates_hz(
            tr.partial_rate_rad_s,
            C_M_PER_S / tr.wavelength_vac_m,
    ...
    def partner_densities_m3(self) -> dict[str, float]:
        """n_s = f_s p/(k_B T) per partner in m^-3 (2.4143e11 m^-3 per partner at 1e-11 mbar and 300 K)."""
    ...
    def collision_coefficients_cm3_s(self, species: Species) -> dict[str, dict[str, float]]:
        """{process: {gas: Gamma_s in cm^3/s}} from the species table; raises ``LookupError`` naming every (process,
        partner) the table does not cite."""
        from qutip_trap.species import MODULES

        table = MODULES[species.name].TABLE if species.name in MODULES else {}
        by_suffix = {cid.partition(".")[2]: c for cid, c in table.items()}
        ...
        for process in COLLISION_PROCESSES:
            for gas in self.gas_fractions:
                c = by_suffix.get(f"D.collision_{process}_{gas}_cm3_s")
                if c is None:
                    missing.append(f"{species.name} {process} by {gas}")
                    continue
                if c.unit != "cm^3/s":
                    raise ValueError(
        ...
        if missing:
            raise LookupError(
                ...
                + "; ".join(missing)
                + " (Knoop et al. 1995 tabulates 40Ca+ with H2 and N2)"
            )
        return out

    def collision_rates_hz(self, species: Species) -> dict[str, float]:
        """{"quench": R_q, "j_mix": R_j} in s^-1, R = sum_s Gamma_s p_s/(k_B T): per ion and density driven; zero when
    ...
    def reshelving_offset(self, tau_s: float) -> float:
        """R/(Gamma + R), the fitted-offset signature of reshelving."""
        if tau_s <= 0.0:
    ...
    def shelf_loss_rates_hz(self, species: Species, shelf: str) -> dict[str, float]:
        """Every rate that empties the metastable level ``shelf``, in s^-1 by channel: its decay, quenching, j-mixing and
        the blackbody transfer along every tabulated M1 line out of it."""
        ...
        rates.update(self.collision_rates_hz(species))
    ...
    def effective_shelf_lifetime_s(self, species: Species, shelf: str) -> float:
        """1/(sum of the shelf's loss rates): a readout scheme's ``shelf_lifetime_s`` with the channels on."""
