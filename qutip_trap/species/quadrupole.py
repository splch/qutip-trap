"""Electric-quadrupole (E2) coupling for optical qubits (James 1998; Roos 2000).

g^(q) contracts eps and k with c^(q) = (2/3) b^(q), never the conjugate duals (which turn q into -q for circular
light). A carrier pi pulse takes t = pi/Omega, so Omega is twice the Boulder value.
"""
...
"""b^(q)_ij with b^(q)_ij r_i r_j = r^2 C^(2)_q, C^(2)_q = sqrt(4 pi/5) Y_{2q} (James 1998 Appendix)."""
...
def racah_c2(q: int, r: Sequence[float]) -> complex:
    """r^2 C^(2)_q(theta, phi) = sqrt(4 pi/5) r^2 Y_{2q} evaluated in Cartesian form at the point ``r``."""
...
def geometric_factor_closed_form(q: int, phi_rad: float, gamma_rad: float) -> float:
    """|g^(q)| (Roos 2000) for B = z, k = (sin phi, 0, cos phi), eps = (cos gamma cos phi, sin gamma, -cos gamma sin phi):
    gamma is the angle between the polarization and the k-B plane."""
...
def decay_weights(J_lower: Half, J_upper: Half) -> dict[tuple[Fraction, Fraction], float]:
    ...
    (the raw sum is 1/6 for every m', so the weights sum to 1 over m at fixed m')."""
...
def quadrupole_collapse_operators(
    ...
    """The E2 collapse operators sqrt(A w(m, m')) |S, m><D, m'| on an internal basis of size ``dimension``.

    ``lower_index`` and ``upper_index`` map m and m' to basis indices; each entry is ((m, m'), A w(m, m'), operator),
    with A = ``partial_rate_rad_s``, the angular partial rate of the E2 line."""
...
def e2_stark_shift_rad_s(
    ...
    """delta_St (rad/s) of the driven |S, m0>-|D, m0'> component, second order in the OTHER components' couplings:

        delta_St = -sum_{m != m0} |Omega(m, m0')|^2/(4 Delta(m, m0')) - sum_{m' != m0'} |Omega(m0, m')|^2/(4 Delta(m0, m'))

    in the (hbar Omega/2) convention, with Delta(m, m') = 2 pi [(E_D(m0') - E_S(m0)) - (E_D(m') - E_S(m))]. The
    denominators carry no i gamma/2, so a component degenerate with the driven one raises ``ZeroDivisionError``."""
...
def rabi_frequency_e2_rad_s(
    ...
    """Omega_{mm'} = (e E_0 k/(2 hbar)) |<S||r^2 C^(2)||D>| |Lambda(m, m')| |g^(q)|, q = m - m'."""
    k = 2.0 * math.pi / wavelength_vac_m
