...
class Species:
    ...
    def rabi_frequency_hz(self, a: str, b: str, beam: Beam, field: Field) -> complex:
        ...
        E1: E_0 sum_q eps_q <b|T_q|a>/hbar in the (hbar Omega/2) convention, complex. E2 (Section 4.5.7): the
        quadrupole scalar (e E_0 k/(2 hbar)) |<S||r^2 C^(2)||D>| |Lambda| |g^(q)|, real, for I = 0 only.
        """
    ...
    def scattering_rates_hz(self, a: str, beam: Beam, field: Field) -> dict[str, float]:
        """Scattering rates in events per second, keyed by the final state's full label (a itself is the Rayleigh entry).

        Despite the ``_hz`` suffix these are rates in s^-1 (photons per second), not angular rates over 2 pi.
        """
        st = self._structure(field)
