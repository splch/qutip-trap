...
The six sections share no state (each rebuilds the fixtures it needs, deterministically, and every Monte Carlo draw is seeded), so
they run in parallel processes and their output is printed in order once every section has finished: the wall time is the longest
section's (section 5, the full calibration, about 24 minutes on the CI runner; section 3's two heating-rate scans took 21) instead
of their sum (48 minutes on the runner, 59% of the first CI job). Per-section wall times go to stderr, which the runner does not
compare. Run: uv run python validation/scripts/check_calibration.py. Lines prefixed MC: are Monte Carlo results.
"""
...
def section_1() -> None:
    header("1. the sideband excitation lineshape (Section 13; Section 9.17 row)")
    omega = TWO_PI * 50e3
    t_pi = math.pi / omega
    print(
    ...
    res = sideband_spectroscopy(
        Machine(dev1), 0, np.linspace(-2.2 * f1, 2.2 * f1, 23), duration_s=0.5 / f1, include_stark=False, fit=True
    )
    print(
...
def section_2() -> None:
    header("2. C0 applied once (Section 9.17 row): sideband-calibrated against carrier-derived eta at q = 0.3")
    dev1 = single_ion_raman_device()
    dd1 = derive_raman_drive(dev1, 0, (0, 1), scattering=False)
    ...
    res = mode_spectroscopy(Machine(dev_rf), 0, 1, include_stark=False, span=0.005, coarse_points=5, fine_points=11)
...
def section_3() -> None:
    header("3. single-ion experiments against the device (Sections 4.1.5, 4.2.7, 7.5)")
    dev1 = single_ion_raman_device()
    th = thermometry(Machine(dev1), 0, 1, nbar={1: 0.3}, include_stark=False, check_durations_s=(31e-6,))
    print(
    ...
    hr = heating_rate(Machine(noisy1), 1, np.linspace(0.0, 3.0 / ndot_true, 6), nbar0=0.1, include_stark=False)
    print(
    ...
    fs = field_scan(Machine(dev1), 0, np.linspace(0.0, 2e-3, 9), include_stark=False, b_seed_gauss=5.03)
    print(
...
def section_4() -> None:
    ...
    header("4. micromotion compensation (Section 7.5; Berkeland 1998)")
    ...
    mm = micromotion_scan(Machine(dev_mm), 0, 0, {"Ex": (-60.0, 0.0)}, method="sideband_ratio", points=5)
    print(
    ...
    corr = micromotion_scan(
        Machine(dev_c), 0, beam, {"Ex": (-40.0, 0.0)}, method="rf_photon_correlation", points=5, rf_points=24
    )
    print(
...
def section_5() -> None:
    header("5. the full calibration of the two-ion 171Yb+ fixture by simulated experiments (Section 7.5)")
    fx = circuit_fixture(2)
...
def section_6() -> None:
    header("6. the servo of Section 7.5")
...
SECTIONS: tuple[Callable[[], None], ...] = (section_1, section_2, section_3, section_4, section_5, section_6)


def run_section(k: int) -> tuple[str, float]:
    """Section ``k`` (1-based) in this process: its stdout captured, and its wall time."""
    buffer = io.StringIO()
    t0 = time.perf_counter()
    with contextlib.redirect_stdout(buffer):
        SECTIONS[k - 1]()
    return buffer.getvalue(), time.perf_counter() - t0


def main() -> None:
    t0 = time.perf_counter()
    # spawn, not fork: every section builds its own QuTiP objects, and the sections' own trajectory maps fork below them
    context = multiprocessing.get_context("spawn")
    with concurrent.futures.ProcessPoolExecutor(max_workers=len(SECTIONS), mp_context=context) as pool:
        futures = [pool.submit(run_section, k) for k in range(1, len(SECTIONS) + 1)]
        for k, future in enumerate(futures, start=1):
            text, wall = future.result()
            sys.stdout.write(text)
            sys.stdout.flush()
            print(f"[check_calibration] section {k}: {wall:.0f} s", file=sys.stderr)
    print(f"[check_calibration] wall {time.perf_counter() - t0:.0f} s over {len(SECTIONS)} parallel sections", file=sys.stderr)
