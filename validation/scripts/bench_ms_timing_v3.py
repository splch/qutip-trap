...
v3 (2026-09-04, evening): closure eta*Omega/eps = 1/2, the maximally entangling ratio in the plan's per-tone
convention H_tone = (hbar Omega/2) sigma_+ e^{...} + h.c. (check_ms_closure.py: concurrence 0.9999 at 1/2, 0.38 at 1/4;
the v2 run's 1/4 was a chi = pi/16 pulse), on the same realizable
two-171Yb+ fixture: omega_x = 3.0, omega_y = 2.9, omega_z = 1.0 MHz; modes x-COM 3.000, x-rock 2.828,
y-COM 2.900 MHz; Raman delta-k in the x-y plane at 30 deg from x; eta = 0.080 (x-COM), +-0.0824 (x-rock,
opposite signs on the two ions), 0.047 (y-COM). v1 used eta*Omega/eps = 1/2 and eta = 0.08, 0.06, 0.05.
"""
...
def ms_hamiltonian(nmodes, nmax, omega_modes, eta, Om, eps):
    ...
    # displacement operator per ion: eta[m][i] carries the ion's sign on rocking modes, exact expm
    terms = [H0]
    mu = omega_modes[0] - eps  # beat-note detuning from carrier
    for i in range(ions):
        gen = sum(1j * eta[m][i] * (a_list[m] + a_list[m].dag()) for m in range(nmodes))
        Dop = gen.expm()
...
def run(nmodes, nmax, T, method):
    omega = [2 * np.pi * 3.0e6, 2 * np.pi * 2.8284e6, 2 * np.pi * 2.9e6][:nmodes]
    eta = [(0.080, 0.080), (0.0824, -0.0824), (0.047, 0.047)][:nmodes]  # per (mode)(ion)
    eps = 2 * np.pi * 10e3  # gate detuning from the sideband, 100 us loop
    Om = eps / (2 * eta[0][0])  # single-loop MS closure in the plan's per-tone convention: eta Omega/eps = 1/2 (K=1), maximally entangling (check_ms_closure.py)
