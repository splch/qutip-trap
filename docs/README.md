...
- [schemas/result.schema.json](schemas/result.schema.json) and [schemas/device.schema.json](schemas/device.schema.json):
  the JSON schemas of `Result.to_dict()` and `Device.to_dict()`, written by `tools/schemas.py` and checked in CI.
