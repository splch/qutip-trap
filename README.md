...
for mixed species, reordered ions). `calibrate(machine, method="experiments")` (`calibration/experiments.py`) runs them in the dependency order
...
**2026-09-09 performance pass.** Every path was profiled and the hot spots rewritten without changing a result beyond the solver
tolerance (ledger family `anchor.perf.*`; `validation/scripts/bench_rotating.py`): (1) every ket segment of the engine now
integrates in the exact rotating frame psi = e^{-i H_0 t} phi of `dynamics/rotating.py`, the diagonal H_mot + H_int removed by
phasing the STATE around the unchanged drive operators (one `QobjEvo` function element per segment over a composite data type,
eigenoperator collapse operators as `[c, e^{i lambda t}]`, expectations and stored states phased back; a segment with a recoil-kick or intensity-noise channel keeps the Schroedinger picture), which cuts the
dop853 evaluations 3 to 8 times on the Section 11.1 rows and runs the real 100 us entangling pulse in 1.0 s instead of 5.1 s at
identical populations (3e-8); `SolverOptions.rotating_frame=False` is the Schroedinger-picture reference and
`SegmentReport.frame` names the picture; (2) the factorized kernel applies the trailing mode factor as one matrix product and
folds its prefactor into a factor (a right-hand side at dimension 2048 from 78 to 56 us); (3) the atomic layer's exact Wigner
algebra is memoized behind one shared `AtomicStructure` per (species, field) (`species.raman.structure_at`), which removes
...
| `qutip_trap/` | the core package (Section 3.2 layout); `qutip_trap/api.py` re-exports the Appendix E surface; the root namespace is rung 0 of the 0.2.0 ladder (`Machine`, `Circuit`, `Result`, the option objects, lazily imported) |
| `qutip_trap/machine.py`, `options.py` | the executor `Machine` (run, compile, schedule, calibrated, estimate, error_model, specs, hash) and the option objects `Physics`, `Numerics`, `Readout` over `SolverOptions` (docs/api_implementation_plan.md 1.3, 1.4); since 0.3.0 `run` is `Machine.run` with its 0.1.0 keywords rewritten under a warning (2.1) |
| `qutip_trap/circuit.py`, `schedule.py`, `physics.py`, `presets.py` | the rung modules of the ladder (rungs 1, 2 and 4, and the example machines); `qutip_trap/dynamics/` is rung 3 |
| `qutip_trap/interop/` | adapters to other SDKs behind optional extras: `qiskit` (a Qiskit 2 `BackendV2` over `run`) |
| `qutip_trap/units.py` | CODATA constants and the `Hz` / `RadPerS` and `Gauss` / `Tesla` types (Sections 5.6, 13) |
| `qutip_trap/_compat.py` | the deprecation helpers of `docs/api_implementation_plan.md` (Phase 0): `QutipTrapDeprecationWarning`, `deprecated` (functions and classes), `deprecated_parameter` with a rewrite of the call, `deprecated_alias`; every warning names its deadline and fix, and the test session turns the warning into an error |
...
| `qutip_trap/benchmarks/error_model.py` | the inverse direction (docs/api_implementation_plan.md 2.6): `ErrorModel` from the gate channels, the SPAM and the schedule, with the IonQ, Quantinuum and QDK exporters and their conversions |
| `qutip_trap/device/serial.py`, `specs.py` | `Device.to_dict`/`from_dict` from the field annotations with `docs/schemas/device.schema.json`, and `Device.specs` (2.5) |
...
| `qutip_trap/io/` | `ionq` (IonQ circuit JSON and v0.4 job bodies, both ways), `openqasm` (the OpenQASM 2 subset importer with custom-gate inlining; M6), `qasm2` (OpenQASM 2 both ways, `loads`/`dumps`) |
