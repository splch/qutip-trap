...
from qutip_trap.species.table import TAGS, IncompleteSpeciesTable
...
NEW_CONSTANTS: tuple[tuple[str, float, float, str, str], ...] = (
    # key, value, uncertainty, tag, source key
...
@pytest.mark.parametrize(("key", "value", "uncertainty", "tag", "source"), NEW_CONSTANTS)
def test_the_constants_entered_carry_value_uncertainty_tag_and_a_located_source(
    key: str, value: float, uncertainty: float, tag: str, source: str
) -> None:
    species_key = key.split(".", 1)[0]
    table = {"ba133": BA133, "ba137": BA137, "be9": BE9}[species_key]
    c = table[key]
    assert c.value == value
    assert c.uncertainty == pytest.approx(uncertainty), f"{key} must carry its published uncertainty"
    assert c.tag == tag and c.tag in TAGS
    assert c.source == source
    assert source in SOURCES, f"{key} cites an unknown source key"
