"""The published models (PLAN.md Section 4): Harty et al.'s randomized benchmarking with the paper's own error model, and
the Molmer-Sorensen, Ballance and Roos closed forms."""
...
def test_harty_error_per_gate_budget() -> None:
    """The model reproduces Harty's error per gate 0.81(14)e-6 (12.1 us pulses, 14 us dead times, +4.5 Hz, a 5e-4 Rabi
    error), 0.3e-6 from the pulse area alone and 0.7e-6 (0.15e-6) from the detuning alone; one identity slot gives less."""
    p = HartyParameters()
...
def test_ms_displacement_carries_the_half_of_the_sideband_coupling() -> None:
    """With the sideband coupling eta Omega/2, |alpha(eps t = pi)| = eta Omega/eps at the maximally entangling closure
    (1e-12) and alpha returns to zero at eps t = 2 pi."""
...
def test_ballance_thermal_error_reproduces_the_printed_cooling_floor() -> None:
    """eps_nbar at eta = 0.123 is 1.1e-5, 2.8e-5, 3.9e-5 and 0.238 at nbar = 0.02, 0.05, 0.06 and 15 to 20 % (the printed
    digits carry Ballance's rounding of eta)."""
