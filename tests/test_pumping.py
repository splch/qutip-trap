"""Optical pumping from first principles (PLAN.md Section 4.2.6; M3): the 171Yb+ pump into |0,0> with its preparation error, duration, photon
count and recoil heating with the ion's participation, the qubit hand-off and the leak policies; the 9Be+-like fixture pumped into |2,2>."""
...
from qutip_trap.prep.pumping import optical_pumping, pumping_time_scale_s, scrambled_initial_state
...
def test_yb171_pump_error_time_photons_and_recoil_heating_with_participation() -> None:
    """Three photons per pump (1/3 branching into |0>), 99.99 % in |0,0> within 10 us at s_o = 0.5, residual 2.5e-6 from the 12.6 GHz off-resonant
    excitation, and the recoil of the three photons heats a 1 MHz mode along B by 8.5e-3 quanta and a 3 MHz mode by 2.8e-3 (Section 4.2.8:
    'Delta n ~ 3e-3' is the quantitative content of the sources' 'Delta n ~ 0')."""
    model = yb_pump_model()
    trap = Trap(
    ...
    assert set(res.photons_per_line) == {"S1/2<-P1/2"}
    ...
    renorm = res.qubit_density_matrix((DARK, "S1/2 F=1 mF=0"), leak="renormalize")
    assert float(np.real(renorm.full()[1, 1])) < float(np.real(rho.full()[1, 1]))
    with pytest.raises(ValueError):
        res.qubit_density_matrix((DARK, "S1/2 F=1 mF=0"), leak="full")
...
def test_zeeman_pumping_on_the_spin_zero_fixture_takes_three_photons() -> None:
    """sigma+ light on S1/2 -> P1/2 of an I = 0 ion pumps |-1/2> into |+1/2>, which sigma+ cannot excite (P1/2 has no m = 3/2): from
    |P1/2, +1/2> the pi decay into the target carries the branching 1/3 and the sigma decay back to |-1/2> 2/3 (Section 4.2.4: 'the pi
    branching out of |P1/2, -+1/2>' is 1/3), so the mean photon number is exactly 3 and the residual is the far-off-resonant P3/2
    excitation only. The two-tone 9Be+ pump into |2,2> (F = 2 and F = 1 tones on one line) is the periodic-frame case of the M3a
    builder and is exercised through the Floquet path, not here."""
    sp = spin_zero_like()
